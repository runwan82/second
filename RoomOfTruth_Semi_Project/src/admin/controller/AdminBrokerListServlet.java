...
public class AdminBrokerListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 업무로직 : 회원목록
		//컨텐츠영역에 대한 쿼리 요청
		List<Broker> list = new BrokerService().selectAll(cPage, numPerPage);
//				System.out.println("list@servlet = " + list);
		
		//페이지바 영역 html
		String url = request.getRequestURI() + "?";// /mvc/admin/memberList?
		int totalContents = new BrokerService().selectTotalContents();
		...
		request.getRequestDispatcher("/WEB-INF/views/admin/brokerList.jsp")
